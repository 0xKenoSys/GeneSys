 ...
 *@param    len     要复制的内容长度， 小于0表示复制offsetSource后面的所有内容
 *@param    fTarget     目标文件，也就是将文件复制到哪里
 *@param    offsetTarget    目标文件的位置偏移，也就是复制到目标文件的什么位置
 *@return   成功复制的字节数
 **/
//long fcopy(FILE *Source, long offsetSource, long len, FILE *fTarget, long offsetTarget) {
...
//int finsert(FILE *fp, long offset, void *buffer, int len) {
    ...
    //if (offset < fileSize) { //从开头或者中间位置插入
        ...
        //fcopy(fp, offset, -1, fpTwmp, offset+len);
