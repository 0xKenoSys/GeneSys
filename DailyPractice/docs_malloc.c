...
//栈由编辑器管，堆由操作系统管
//malloc的函数签名是void* malloc(size_t size);
//拿到这块内存之后，必须自己做“类型强转”。比如想存整数，要写成 (int*)malloc(n * sizeof(int))
//永远不要硬编码具体的字节数（比如假设 int 是 4 字节所以写 4*n），因为在不同的硬件架构下，数据类型的长度是会变的。
//致命考点： 操作系统不是无限资源的。如果堆内存碎片化严重或者已经被耗尽，malloc 会申请失败。一旦失败，它会返回一个 NULL 空指针。如果你不检查返回值，直接对 NULL 指针进行写入操作，程序会当场暴毙。一个合格的工程师，每次调用 malloc 后紧接着的第一行代码，必然是检查指针是否为 NULL。
//当你调用 free(ptr) 时，底层库会将指针偷偷往回退几个字节，读取那个隐藏的长度信息，然后精准地释放掉整块内存。这也引出了一个极其危险的操作： 你传给 free 的指针，必须绝对、完全等于当初 malloc 返回给你的那个指针。
//在 free(ptr); 的下一行，立刻写上 ptr = NULL;
